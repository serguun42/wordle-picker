...
self.addEventListener('install', (e) => {
  e.waitUntil(caches.open(process.env.REACT_APP_CACHE_STORAGE_NAME).then((cache) => cache.addAll(['/'])));
});
...
self.addEventListener(
  ...
  /** @param {Event & { request: Request, respondWith: (Promise<Response>) => void }} event */ (event) => {
    ...
    if (parsedURL.pathname.search('/build_hash') > -1) return fetch(request);
